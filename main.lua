...
DefaultSettings = (function()
    ...
    return {
        ...
        Theme = Theme and {
            ...
            Main1 = Theme.Main1,
            Main2 = Theme.Main2,
            Outline1 = Theme.Outline1,
            Outline2 = Theme.Outline2,
            Outline3 = Theme.Outline3,
            TextBox = Theme.TextBox,
            Menu = Theme.Menu,
            ListSelection = Theme.ListSelection,
            Button = Theme.Button,
            ...
            Syntax = {
                ...
                Selection = Theme.Syntax.Selection,
                SelectionBack = Theme.Syntax.SelectionBack,
                ...
                FindBackground = Theme.Syntax.FindBackground,
                MatchingWord = Theme.Syntax.MatchingWord,
                BuiltIn = Theme.Syntax.BuiltIn,
                CurrentLine = Theme.Syntax.CurrentLine,
                LocalMethod = Theme.Syntax.LocalMethod,
                LocalProperty = Theme.Syntax.LocalProperty,
                Nil = Theme.Syntax.Nil,
                Bool = Theme.Syntax.Bool,
                Function = Theme.Syntax.Function,
                Local = Theme.Syntax.Local,
                Self = Theme.Syntax.Self,
                FunctionName = Theme.Syntax.FunctionName,
...
Main = (function()
    ...
    Main.ModuleList = {"Theme", "Explorer", "Properties", "ScriptViewer", "Console", "SaveInstance", "ModelViewer"}
    Main.Elevated = false
    ...
    Main.LoadModule = function(name)
        if Main.Elevated then -- If you don't have filesystem api then ur outta luck tbh
            ...
            elseif _G.DebugLoadModel then -- Load Debug Model File
                ...
                if not hashs then
                    local s, hashDataStr = pcall(oldgame.HttpGet, game, "https://api.github.com/repos/" ..
                        Main.GitRepoName .. "/ModuleHashs.dat")
                    if not s then
                ...
                if s and hashfunc(moduleStr) == hashs[name] then
                    ...
                    -- Download and cache
                    local s, moduleStr = pcall(oldgame.HttpGet, game, "https://api.github.com/repos/" ..
                        Main.GitRepoName .. "/Modules/" .. name .. ".lua")
                    if not s then
    ...
    Main.InitEnv = function()
        ...
        env.loadstring = (pcall(loadstring, "local a = 1") and loadstring) or
                             (game:GetService("RunService"):IsStudio() and script.Modules:FindFirstChild("Loadstring") and
                                 require(script.Modules:FindFirstChild("Loadstring")))
        ...
        env.saveinstance = saveinstance or (function()
            ...
            local synsaveinstance = loadstring(oldgame:HttpGet(Params.RepoURL .. Params.SSI .. ".luau", true),
                Params.SSI)()
        ...
        env.isViableDecompileScript = function(obj)
            if obj:IsA("ModuleScript") then
                return true
            elseif obj:IsA("LocalScript") and
                (obj.RunContext == Enum.RunContext.Client or obj.RunContext == Enum.RunContext.Legacy) then
                return true
        ...
        env.decompile = decompile or (function()
            ...
            if not env.getscriptbytecode then --[[warn('Konstant decompiler is not supported. "getscriptbytecode" is missing.')]]
                return
            end
    ...
    Main.IncompatibleTest = function()
        --[[local function incompatibleMessage(reason, tolerated)
			...
			t.BackgroundColor3 = Settings.Theme.Main2
    ...
    local function deserialize(val)
        if typeof(val) == "table" then
            if val.R and val.G and val.B then
                return Color3.fromRGB(val.R * 255, val.G * 255, val.B * 255)
            else
                return val
            end
        else
    ...
    Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
        local gui = create({{1, "ScreenGui", {
            ...
            ImageColor3 = Settings.Theme.Main2,
            Parent = {8},
            ScaleType = 1,
            Size = UDim2.new(1, 0, 1, 0),
    ...
    Main.CreateMainGui = function()
        local gui = create({{1, "ScreenGui", {
            ...
            ScrollBarImageColor3 = Settings.Theme.Outline3,
            ...
            Text = "",
            TextColor3 = Settings.Theme.Outline3,
            TextSize = 14
        }}, {19, "ImageLabel", {
            BackgroundColor3 = Settings.Theme.Text,
            BackgroundTransparency = 1,
        ...
        openButton.MainFrame.BottomFrame.Information.MouseButton1Click:Connect(function()
            local duration = 1
            local Infos = {"Contributors >>", "Toon (IY Dex and PRs)", "Moon (Dex)", "Cazan (3D Preview)"}
    ...
    Main.Init = function()
        ...
        if Main.Elevated then
            ...
            Main.RobloxVersion = Main.RobloxVersion or
                                     oldgame:HttpGet(
                    "https://clientsettings.roblox.com/v2/client-version/WindowsStudio64/channel/LIVE"):match(
                    "(version%-[%w]+)")
        end
