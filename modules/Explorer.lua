...
local function main()
    ...
    moveObject = function(obj)
        ...
        elseif nilMap[newPar.Obj] or newPar == nilNode then
            nilMap[obj] = true
            nilCons[obj] = nilCons[obj] or
                               {connectSignal(obj.ChildAdded, addObject), connectSignal(obj.AncestryChanged, moveObject)}
        end
    ...
    Explorer.Refresh = function()
        ...
        for i = 1, maxNodes do
            ...
            if node then
                ...
                entry.Indent.EntryName.TextTruncate = (Settings.Explorer.UseNameWidth and Enum.TextTruncate.None or
    ...
    Explorer.ShowRightClick = function(MousePos)
        ...
        if presentClasses["LuaSourceContainer"] then
            ...
            context:AddRegistered("SAVE_SCRIPT", not presentClasses.isViableDecompileScript or env.decompile == nil or
                env.writefile == nil)
            context:AddRegistered("SAVE_BYTECODE",
                not presentClasses.isViableDecompileScript or env.getscriptbytecode == nil or env.writefile == nil)
    ...
    Explorer.InitRightClick = function()
        ...
        context:Register("SAVE_INST", {
            ...
            OnClick = function()
                local sList = selection.List
                if #sList == 1 then
                    Lib.SaveAsPrompt("Place_" .. game.PlaceId .. "_" .. sList[1].Obj.ClassName .. "_" ..
                                         sList[1].Obj.Name .. "_" .. os.time(), function(filename)
                ...
                elseif #sList > 1 then
                    for i = 1, #sList do
                        ...
                        Lib.SaveAsPrompt("Place_" .. game.PlaceId .. "_" .. sList[i].Obj.ClassName .. "_" ..
                                             sList[i].Obj.Name .. "_" .. os.time(), function(filename)
        ...
        context:Register("COPY_API_PAGE", {
            ...
            OnClick = function()
                local sList = selection.List
                if #sList == 1 then
                    env.setclipboard("https://create.roblox.com/docs/reference/engine/classes/" ..
            ...
            OnClick = function()
                for _, v in next, selection.List do
                    if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
                        ...
                        local fileName = ("%s_%s_%i_Source.txt"):format(env.parsefile(v.Obj.Name), v.Obj.ClassName,
                            game.PlaceId)
                ...
                for _, v in next, selection.List do
                    if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
                        ...
                        if success and type(bytecode) == "string" then
                            local fileName = ("%s_%s_%i_Bytecode.txt"):format(env.parsefile(v.Obj.Name),
                                v.Obj.ClassName, game.PlaceId)
    ...
    Explorer.SearchFilters = { -- TODO: Use data table (so we can disable some if funcs don't exist)
        Comparison = {
            ...
            ["rad"] = function(argString)
                ...
                if not service.Players.LocalPlayer.Character or
                    not service.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or
                    not service.Players.LocalPlayer.Character.HumanoidRootPart:IsA("BasePart") then
                    return
                end
...
return search]==]
    ...
    Explorer.InitEntryTemplate = function()
        entryTemplate = create({{1, "TextButton", {
            ...
            BackgroundColor3 = Theme.DefaultDark,
            BackgroundTransparency = 1,
            BorderColor3 = Theme.DefaultDark,
            Font = 3,
            Name = "Entry",
            ...
            BackgroundTransparency = 1,
            BorderColor3 = Theme.AccentBlue2,
            BorderSizePixel = 0,
            Name = "Indent",
            Parent = {1},
    ...
    Explorer.UpdateSelectionVisuals = function()
        ...
        if not holder then
            ...
            boxTemplate.Color3 = Theme.BrightBlue
        ...
        for i, v in pairs(Explorer.SelectionVisualGui:GetChildren()) do
            v.BackgroundColor3 = Theme.BrightBlue
        end
    ...
    Explorer.Init = function()
        ...
        if Settings.ClassIcon ~= nil and Settings.ClassIcon ~= "Old" then
            ...
            Explorer.ClassIcons = Lib.IconMap.new("rbxassetid://" .. tostring(iconData.MapId),
                iconData.IconSize * iconData.Witdh, iconData.IconSize * iconData.Height, iconData.IconSize,
                iconData.IconSize)
        ...
        local explorerItems = create({{1, "Folder", {
            ...
            ImageColor3 = Theme.PlaceholderText,
            Parent = {7},
            Size = UDim2.new(0, 16, 0, 16)
        }}, {9, "TextButton", {
            AutoButtonColor = false,
            ...
            BackgroundColor3 = Theme.ButtonPress,
            BorderSizePixel = 0,
            Name = "ScrollCorner",
            Parent = {1},
