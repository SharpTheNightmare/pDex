...
local function main()
    ...
    ScriptViewer.DumpFunctions = function(scr)
        ...
        function functions:dump_table(input, indent, index)
            ...
            functions:add_to_dump(("%s [%s] %s"):format(tostring(index), tostring(typeof(input)), tostring(input)),
                indent - 1)
            local count = 0
            for index, value in pairs(input) do
                count = count + 1
                if type(value) == "function" then
                    functions:add_to_dump(("%d [function] = %s"):format(count, functions:get_function_name(value)),
                        indent)
                elseif type(value) == "table" then
                    ...
                    functions:add_to_dump(("%d [%s] = %s"):format(count, tostring(typeof(value)), tostring(value)),
                        indent)
                end
            end
        end
        function functions:dump_function(input, indent)
            ...
            for index, upvalue in pairs(getupvalues(input)) do
                if type(upvalue) == "function" then
                    functions:add_to_dump(("%d [function] = %s"):format(index, functions:get_function_name(upvalue)),
                        indent + 1)
                elseif type(upvalue) == "table" then
                    ...
                    functions:add_to_dump(("%d [%s] = %s"):format(index, tostring(typeof(upvalue)), tostring(upvalue)),
                        indent + 1)
                end
            end
            ...
            for index, constant in pairs(getconstants(input)) do
                if type(constant) == "function" then
                    functions:add_to_dump(("%d [function] = %s"):format(index, functions:get_function_name(constant)),
                        indent + 1)
                elseif type(constant) == "table" then
                    ...
                    functions:add_to_dump(
                        ("%d [%s] = %s"):format(index, tostring(typeof(constant)), tostring(constant)), indent + 1)
                end
            end
        end
    ...
    ScriptViewer.Init = function()
        ...
        if env.setclipboard then
            copy.TextColor3 = Theme.TextColor
            copy.Interactable = true
        else
            copy.TextColor3 = Theme.PlaceholderText
        ...
        save.TextColor3 = Theme.TextColor

        if env.writefile then
            save.TextColor3 = Theme.TextColor
            save.Interactable = true
        else
            save.TextColor3 = Theme.PlaceholderText
        ...
        if env.getgc then
            dumpbtn.TextColor3 = Theme.TextColor
        ...
        execute.TextColor3 = Theme.TextColor

        if env.loadstring then
            execute.TextColor3 = Theme.TextColor
            execute.Interactable = true
        else
            execute.TextColor3 = Theme.PlaceholderText
        ...
        clear.Text = "Clear"
        clear.TextColor3 = Theme.TextColor
    ...
    ScriptViewer.ViewScript = function(scr)
        ...
        if not s or not source then
            ...
            if (scr.ClassName == "Script" and
                (scr.RunContext == Enum.RunContext.Legacy or scr.RunContext == Enum.RunContext.Server)) or
